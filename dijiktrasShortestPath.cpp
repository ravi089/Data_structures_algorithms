// Graph algorithms.
// Adjacency matrix represntation for
// the graph.
...
void dijiktraShortestPath( vector<string> s )
{
	int N = s.size();
	g.resize(N);
	vector<bool> intree;
	...
	while ( intree[cVertex] == false ) {
		...
		for ( it = g[cVertex].begin(); it != g[cVertex].end(); ++it ) {
			...
			if ( distance[nVertex] > distance[cVertex] + weight )
				distance[nVertex] = distance[cVertex] + weight;
		}
		cVertex = 0;
		int dist = INT_MAX;
		for ( int i = 0; i < N; i++ ) {
